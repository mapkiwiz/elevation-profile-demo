...
export logger = Logger.get("APP")

export set_log_level = (level) ->
  if level not in Logger.log_levels
    console.log("[APP] unrecognized logging level '#{level}' to set_log_level(), ignoring")
    console.log("[APP] valid log levels are: #{Logger.log_levels.join(', ')}")
  else
    console.log("[APP] setting log level to: '#{level}'")
